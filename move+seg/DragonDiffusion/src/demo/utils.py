...
def get_point_move(original_image, img, sel_pix, evt: gr.SelectData):
    print(f"_______Sel pix{sel_pix}")
    if original_image is not None:
    ...
    for idx, point in enumerate(sel_pix):
        ...
        if len(points) == 2:
            ...
            points = []
    img = np.array(img)
    cv2.imwrite("img_from_get_move.png",img)
    cv2.imwrite("orginalimg_from_get_move.png",original_image)
    print(sel_pix)
...
def store_img_move(img):
    print(f"store_img_move_img{type(img)}")
